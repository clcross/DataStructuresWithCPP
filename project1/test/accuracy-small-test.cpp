...
#include <time.h>
#include "map.hpp"
#include "ans-small.hpp"

class MyTestSuite1 : public CxxTest::TestSuite {
    //const char[][] answers = {"test"};
        Map map;
    public:
        void gen(std::vector<std::string> &first, std::vector<std::string> &last) {
            printf("const char * const answers[] = {\n");

            for (int i = 0; i < first.size(); i+=100) {
                for (int j = 0; j < last.size(); j+= 100) {
                    if (i != 0 || j != 0) {
                        printf(", ");
                    }
                    printf("\"%s %s\"", first[i].c_str(), last[j].c_str());
                }
            }
